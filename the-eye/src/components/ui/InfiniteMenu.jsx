...
class ArcballControl {
  ...
  update(deltaTime, targetFrameDuration = 16) {
    ...
    // Auto-rotate logic
    if (this.autoRotate && this.autoRotateTarget) {
      this.autoRotateProgress += deltaTime / 1000; // Convert to seconds
      const t = Math.min(this.autoRotateProgress, 1);
      const easeT = t < 0.5 ? 2 * t * t : -1 + (4 - 2 * t) * t; // Ease in-out
      
      quat.slerp(this.orientation, this.orientation, this.autoRotateTarget, easeT * 0.05);
      
      if (this.autoRotateProgress >= 1) {
    ...
    if (this.isPointerDown) {
      ...
      // Increment auto-rotate timer
      this.autoRotateTimer += deltaTime;
      
      // Start auto-rotation after 5 seconds of inactivity
      if (this.autoRotateTimer > 5000 && !this.autoRotate) {
        this.startAutoRotate();
        this.autoRotateTimer = 0; // Reset timer for next rotation
      }
      
      const INTENSITY = 0.1 * timeScale;
      quat.slerp(this.pointerRotation, this.pointerRotation, this.IDENTITY_QUAT, INTENSITY);

      if (this.snapTargetDirection && !this.autoRotate) {
  ...
  startAutoRotate() {
    // Create a random rotation target
    const randomAxis = vec3.normalize(vec3.create(), [
      Math.random() - 0.5,
      Math.random() - 0.5,
      Math.random() - 0.5
    ]);
    const randomAngle = (Math.random() * Math.PI * 0.5) + Math.PI * 0.25; // Random angle between 45-135 degrees
    
    this.autoRotateTarget = quat.create();
    quat.setAxisAngle(this.autoRotateTarget, randomAxis, randomAngle);
    quat.multiply(this.autoRotateTarget, this.autoRotateTarget, this.orientation);
