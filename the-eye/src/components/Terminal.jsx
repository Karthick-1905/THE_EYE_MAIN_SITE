...
function Terminal({ onClose, id }) {
  ...
  const containerRef = useRef(null); // terminal container for animations
...
"   ╚═╝   ╚═╝  ╚═╝╚══════╝    ╚══════╝    ╚═╝   ╚══════╝",
 ...
 // Auto-scroll to bottom on new log entries
  ...
  useEffect(() => {
    ...
    if (el) {
      el.classList.add('terminal-enter');
      // force reflow
      // eslint-disable-next-line no-unused-expressions
      el.offsetHeight;
      el.classList.add('terminal-enter-active');
    }
  ...
  return (
    <div
      ...
      role="region"
      aria-label="THE EYE interactive terminal"
      ref={containerRef}
    >
      <div className="terminal-header">
        <span>THE EYE Terminal</span>
        <button onClick={onClose} aria-label="Close terminal" title="Close">✕</button>
      </div>
